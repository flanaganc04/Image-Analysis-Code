...
ret, thresh = cv.threshold(imgray, 185, 255, 0)
...
for contour in contours:
    if 300 > contour.size > 50:
        filtered_contour = filter_contour_by_y_level(contour, 0, 24)
        print(contour.size)
        ...
        drawn = cv.drawContours(image, [left_contour], -1, (255, 240, 0), 2)
        drawn = cv.drawContours(image, [right_contour], -1, (255, 240, 0), 2)
...
cv.imwrite(f'OT-2 Image analysis/newBackground/{number}drawn.JPG', drawn)