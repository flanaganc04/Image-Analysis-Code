...
path = 'OT-2 Image analysis/benchMarking/toluene, silicon/Toluene.Silicon2.JPG'
img = cv.imread(path)
print(img.shape)

preprop = Preprocess(path)
preprop.process_image('OT-2 Image analysis/benchMarking/toluene, silicon/binary2.png')
binary = cv.imread('OT-2 Image analysis/benchMarking/toluene, silicon/binary2.png')
binary = binary[1900:2000, 825:1200]

crop = img[1900:2000, 825:1200]

imgray = cv.cvtColor(crop, cv.COLOR_BGR2GRAY)
ret, thresh = cv.threshold(imgray, 253, 255, 0)
...
for contour in contours:
    if 400> contour.size > 200:
        print(contour.shape)
        print(contour.size)
        # cv.drawContours(binary, [contour], 0, (0, 0, 255), 2)
        contour1 = contour[32:63]
        contour2= contour[151:172]
...
cv.imwrite('OT-2 Image analysis/benchMarking/toluene, silicon/angles2.png', binary)
